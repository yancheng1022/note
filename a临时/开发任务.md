...
1. 测试表：ims_test_result
   分表字段：test_uid，submit_work_time
   清洗字段：test_uid，submit_work_time，sample_uid
   默认表：ims_test_result_200001
2. 工单表：ims_work_result
   分表字段：uid，submit_work_time
   清洗字段：uid，relate_work_uid，sample_uid，test_item_ids
   默认表：ims_work_result_200001
3. 取样表：ims_test_sample_fetch
   分表字段：submit_work_time,test_uid
   清洗字段：submit_work_time,test_uid
   默认表：ims_test_sample_fetch_200001
4. 样本稀释表：ims_test_sample_diluent
   分表字段：submit_work_time,test_uid
   清洗字段：submit_work_time,test_uid
   默认表：ims_test_sample_diluent_200001
5. 试剂添加表：ims_test_reagent_add
   分表字段：submit_work_time,test_uid
   清洗字段：submit_work_time,test_uid
   默认表：ims_test_reagent_add_200001
6. 通用液添加表：ims_test_ls_add
   分表字段：submit_work_time,test_uid
   清洗字段：submit_work_time,test_uid
   默认表：ims_test_ls_add_200001
7. 温浴表：ims_test_incubate
   分表字段：submit_work_time,test_uid
   清洗字段：submit_work_time,test_uid
   默认表：ims_test_incubate_200001
8. 读数表：ims_test_read
   分表字段：submit_work_time,test_uid
   清洗字段：submit_work_time,test_uid
   默认表：ims_test_read_200001
