...
14. 根据LTL和UTL截断数据。数据被截断并且是定标点，将前一个点设置为定标点
15. 数据转换，使数据更符合正态性(0-Box-Cox变换、1-对数变换、2-倒数变换、3-平方根变换)
16. 记录最优lambda，加权失控报警0.5N和2N最优lambda用N计算出的lambda
17. 计算本次控制线
18. 统计值计算(0-MA、1-MQ、2-EWMA、3-MovSD、4-MR)
19. 计算本次控制线（LCL,UCL）
20. 判断控制线的合理性：根据控制线偏差和Moving-Slope斜率最大值，最小值判断（斜率判断最新版本已废除）
21. 满足合理性判断时，暂存N转换后的数据,用作失控报警中的加权失控计算（0.5N，2N）
22. 未找到合适控制线时，直接返回并提示用户
23. 使用计算控制线得到的LTL和UTL截断全量数据
24. 
