...
aof写数据策略：
**always**：每次写入操作都同步到aof文件中always
**everysec**：每秒将缓冲区中的指令同步到aof中（默认配置）
**no**：由操作系统控制写入aof的周期
...
### 4.2.3、定期删除
方式：redis启动服务器初始化时,读取配置server.hz的值,默认是10

优缺点：
（1）每秒花费固定cpu资源维护内存。内存友好方面，不如"定时删除"，在CPU时间友好方面，不如"惰性删除"
