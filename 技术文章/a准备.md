...
1. 按数据结构分类
B-Tree索引：最常用的索引类型，适用于全键值、键值范围或键前缀查找，InnoDB和MyISAM引擎默认使用B+Tree实现
哈希索引：基于哈希表实现，只有精确匹配所有列的查询才有效，Memory引擎默认索引类型
全文索引：用于全文搜索，MyISAM和InnoDB(5.6+)支持，使用倒排索引实现
R-Tree索引：用于空间数据索引，MyISAM支持空间索引

2. 按物理存储分类
聚簇索引(Clustered Index)：InnoDB的主键索引，索引的叶节点存储了完整的数据记录，表数据本身就是索引的一部分
