...
随着智慧城市与安全生产建设的深化，公共场所的违规抽烟行为管控已成为保障公共安全与健康的关键环节。传统人工监控方式存在效率低、覆盖窄的瓶颈。尽管基于深度学习的目标检测技术，特别是以YOLO系列为代表的实时检测算法，为此提供了可行的技术路径，但针对“抽烟”这一特定行为，仍面临香烟目标细小、姿态多变、遮挡严重及复杂场景干扰等核心挑战。因此，开发一个能够精准、实时识别抽烟行为的智能化检测系统，具有重要的现实意义与应用价值。
## 📌 2、数据集概览
