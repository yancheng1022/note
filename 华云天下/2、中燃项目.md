...
FROM
  s_cc_gas_stop c
  LEFT JOIN gas_stop g ON c.gas_stop_id = g.gas_stop_id UNION ALL
SELECT
  g.gas_stop_id,
  g.org_no,
  g.gas_stop_no,
...
李总，企微群消息太多被淹了，停气管理有问题要确认下：
1、新增里的预计结束时间和停气恢复时间什么区别？目前从crm拿到的数据是 预计开始/结束时间，实际开始/结束时间 这四个字段。这个停气恢复时间对应crm里的哪个时间
2、钉钉推送的数据模板需要提供一下
