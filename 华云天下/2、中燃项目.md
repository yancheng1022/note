...
上周做了中燃用户数据同步和处理铁路局bug。这周先做中燃用户分配租户的功能，然后开始做中燃短信对接的功能


中燃用户租户分配、组织结构查询 2天   短信对接 2天


主数据同步有两个问题还需要确认下
1、默认用户类型 （1.管理,2.在线,3.热线,4.在线+热线）
2、默认角色
...
CREATE TABLE `iam_sync_batch` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='IAM同步批次信息表';
...
) ENGINE=InnoDB AUTO_INCREMENT=15532 DEFAULT CHARSET=utf8;
...
) ENGINE=InnoDB AUTO_INCREMENT=48284 DEFAULT CHARSET=utf8;
```