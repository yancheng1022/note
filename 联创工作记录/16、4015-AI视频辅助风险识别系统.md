...
开放神经网络交换 ONNX（Open Neural Network Exchange）使用不同框架训练的模型，转化为 ONNX 格式后，可以很容易的部署在兼容 ONNX 的运行环境中。
...
1、视频的帧率 fps是 30，，也就是说每秒会播放30帧的图像 ，隔3帧推理一次， 也有10帧
一帧图像只有 1000ms/10=100ms时间去完成拉流  推理 保存视频帧等业务逻辑，才能保证不会堆积。不然就会出现推流和业务逻辑延迟的情况

目前一次推理300ms，加上报警视频保存时间更长（2000ms）

2、目前模型都是别人的，所以入参和响应格式不统一，无法做到统一处理，都是写各自的算法实现的。所以页面增加算法完成后并不能实现对应的功能


3、安全帽模型不准确

4、海康换地址问题还没考虑，需要完善这块逻辑
