...
| 2   | 领导信息上传错误导致带班空岗 | 114  | QueueExcute.handlePersonReal |      |      |
| 3   | 数据上传种类是否齐全     | 115  |                              |      |      |
| 4   | 是否存在超员         | 116  |                              |      |      |
| 5   | 是否存在疑似睡岗       | 117  |                              |      |      |
// 核查单通知 - 人员  
List<DataChecklistEntity> checkCollect = rst.stream().filter(item -> "不通过".equals(item.getChecklistResult())).collect(Collectors.toList());  
for (DataChecklistEntity item : checkCollect){  
    if ("基础信息文件每天上传一次".equals(item.getChecklistItem())){  
        noticeAlarmSettingService.checkListNotice("RY", "101", "基础信息文件每天上传一次", item.getChecklistDesc());  
    }else if ("领导带班空岗".equals(item.getChecklistItem())){  
        noticeAlarmSettingService.checkListNotice("RY", "114", "领导带班空岗", item.getChecklistDesc());  
    }else if ("数据上传是否完整".equals(item.getChecklistItem())){  
        noticeAlarmSettingService.checkListNotice("RY", "115", "数据上传种类不齐全", item.getChecklistDesc());  
    }else if ("超员分析".equals(item.getChecklistItem())){  
        noticeAlarmSettingService.checkListNotice("RY", "116", "存在超员", item.getChecklistDesc());  
    }else if ("疑似睡岗".equals(item.getChecklistItem())){  
        noticeAlarmSettingService.checkListNotice("RY", "117", "存在疑似睡岗", item.getChecklistDesc());  
    }  
}
