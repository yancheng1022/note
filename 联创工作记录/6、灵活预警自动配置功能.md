...
| :-- | :-------------------------- | :--- | -------------------------------------- | --------- |
| 1   | 基础信息文件每天上传一次                | 100  | DataChecklistTask.baseInfoFileCheck    | 完成        |
| 2   | 空岗                          |      | PersonVacancyServiceImpl.saveVacanInfo | 空岗判断逻辑有疑问 |
| 3   | 中断                          |      | ReportYjInterruptTask.sendYjSms        | 完成        |
| 4   | 中断                          |      | ReportYjInterruptTask.sendYjSms        |           |
| 5   | 报警文件中不存在实时数据                |      |                                        |           |
| 6   | 报警文件未上传报警结束时间               |      |                                        |           |
| 7   | 数据上传种类是否齐全                  |      |                                        |           |
| 8   | 信息不全：无传感器地点名称               |      |                                        |           |
| 9   | 信息不全：无报警门限值                 |      |                                        |           |
| 10  | 信息不全：无传感器类型                 |      |                                        |           |
| 11  | 信息不全：传感器命名相同或为其它或为空，或为模糊性地点 |      |                                        |           |
| 12  | 领导信息上传错误导致带班空岗              |      |                                        |           |
| 13  | 数据上传种类是否齐全                  |      |                                        |           |
| 14  | 是否存在超员                      |      |                                        |           |
| 15  | 是否存在疑似睡岗                    |      |                                        |           |
